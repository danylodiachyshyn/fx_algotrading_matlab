% test_ForwardLookingAnalysis_DoubleEMA_ATR
clear, clc, close all
...
length1   = 10;
factorIS  = 372;   % 3720 bars = ~620 Days (2.5 years of macro memory)
factorOoS = 90;

disp('Running Walk-Forward Analysis (Double EMA + ATR)...');
[ECOoS, Results] = my_ForwardLookingAnalysisDoubleEMA_ATR(...
    data, FTSname2, TF, Currency, periodFastRange, periodSlowRange, periodATRRange, ...
    atrThresholdRange, SLRange, TPRange, length1, factorIS, factorOoS, 1);

disp(' ');
disp('--- Walk-Forward Results Table ---');
disp(Results);